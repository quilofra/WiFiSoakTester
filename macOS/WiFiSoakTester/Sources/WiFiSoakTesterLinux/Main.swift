...
private actor ConsoleReporter {
    ...
    func printStart(configuration: DownloadConfiguration, urls: [URL]) {
        print("WiFiSoakTesterLinux started")
        print("Endpoints: \(urls.count) | Concurrency: \(configuration.concurrency) | Sink: \(configuration.sinkMode.rawValue)")
        print("Duration: \(durationText(configuration)) | Timeout: \(configuration.timeoutSeconds)s | Retries: \(configuration.maxRetries)")
        print("Type 'stop' + Enter to finish gracefully.")
        print("")
    }
    ...
    func printSummary(samples: [SessionSample]) {
        guard !samples.isEmpty else {
            print("Finished without samples.")
            return
        }
        ...
        print("Total: \(ValueFormatting.bytes(total))")
        print("Average: \(ValueFormatting.speedMbps(average))")
        print("p50/p95: \(ValueFormatting.speedMbps(p50)) / \(ValueFormatting.speedMbps(p95))")
    }
...
struct WiFiSoakTesterLinux {
    ...
    private static func exportIfRequested(
        ...
        if let csvPath = options.csvOut, !csvPath.isEmpty {
            ...
            print("INFO: CSV exported to \(csvURL.path)")
        }
        ...
        if let htmlPath = options.htmlOut, !htmlPath.isEmpty {
            ...
            print("INFO: HTML report exported to \(htmlURL.path)")
        }
    }

    private static func printHelp() {
        print("""
WiFiSoakTesterLinux (CLI)

Usage:
  WiFiSoakTesterLinux [options]
